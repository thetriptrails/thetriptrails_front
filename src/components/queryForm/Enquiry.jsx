...
const Field = ({ label, icon, error, children }) => (
  <div className="flex flex-col gap-0.5">
    {label && (
      <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-stone-400 ml-0.5">
...
const inputCls =
  "w-full px-3 py-2 rounded-xl border border-stone-200 bg-white/70 " +
  ...
  return (
    <section className="w-full max-w-2xl mx-auto py-4 px-4">
      ...
      >
        ...
        <div className="px-6 pt-6 pb-2 text-center relative z-10" style={{ background: `linear-gradient(160deg, ${LIGHT} 0%, white 100%)` }}>
          <h2 className="text-2xl font-bold text-stone-800 tracking-tight">{title}</h2>
          <Ornament />
          <p className="text-stone-500 text-xs mt-1 font-sans max-w-sm mx-auto">{subtitle}</p>
        </div>

        {/* Form Body - Reduced padding for mobile */}
        <div className="px-6 pb-6 pt-3 z-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          {submitted ? (
            <div className="flex flex-col items-center justify-center py-6 text-center animate-in fade-in zoom-in duration-500">
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg text-xl mb-3" style={{ background: `linear-gradient(135deg, ${GOLD}, ${DARK})` }}>✓</div>
              <h3 className="text-lg font-bold text-stone-800 italic">Inquiry Sent</h3>
              <p className="text-stone-500 text-xs mt-1">We will get back to you shortly.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <GradBtn type="submit" loading={loading} className="w-full py-3 rounded-xl text-white font-bold uppercase tracking-widest text-xs shadow-lg mt-1">
                {submitLabel}
              </GradBtn>
            </form>
          )}
